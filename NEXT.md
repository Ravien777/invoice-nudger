# NEXT.md — Maroni: Complete Finance Solution
...
4. Render a clean, professional page (no Maroni branding unless white-label is off):
...
**Goal:** Make Maroni installable on a phone and fast on mobile. This is a polish phase — no new features, just PWA setup and mobile layout fixes.
...
1. Add `receiptEmail String? @unique` to `User`. Format: `receipts.[userid]@mail.getmaroni.com` (or whatever your sending domain is).
...
1. `POST /api/accountant/invite`: accept `{ email }`. Create `AccountantAccess` record. Send an email to the accountant via Resend: `"[YourName] has invited you to view their Maroni account. Click here to accept."` with link `https://yourapp.com/accountant/accept/[inviteToken]`.
...
1. Generate a clean one-page PDF using `@react-pdf/renderer`:
   ...
   - A note: `"Generated by Maroni. For reference purposes only."`

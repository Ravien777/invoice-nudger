...
export default function TaxClient({
  ...
  const [savingsInput, setSavingsInput] = useState(String(Math.round(initialTaxSavings)));
  ...
  const handleSaveSavings = async () => {
    ...
    try {
      ...
      if (data) setData({ ...data, taxSavingsAmount: Number(savingsInput) || 0 });
    } finally {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="rounded-xl border border-border-default bg-surface p-6 animate-pulse">
        ...
        <>
          ...
          {isPro ? (
            <div className="rounded-xl border border-border-default bg-surface p-5">
              <h3 className="text-sm font-medium text-text-primary mb-3">Set Aside Tracker</h3>
              ...
              <div className="flex items-center gap-2 max-w-xs">
                ...
                />
                <Button size="sm" onClick={handleSaveSavings} loading={savingSavings}>
                  Update
                </Button>
              </div>
            </div>
          ...
          <div className="rounded-xl border border-border-default bg-surface overflow-hidden">
            ...
            >
              <h3 className="text-sm font-medium text-text-primary">Profit & Loss Report</h3>
            ...
            {showPnL && pnl && (
              <div className="border-t border-border-default p-5 space-y-6">
                ...
                {/* Expenses section */}
                <div>
                  <h4 className="text-xs font-semibold text-text-tertiary uppercase tracking-wider mb-2">Expenses by Category</h4>
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-text-tertiary text-xs uppercase border-b border-border-default">
          ...
          <div className="rounded-xl border border-[var(--warning-muted)] bg-[var(--warning-muted)]/10 p-4">
            <p className="text-sm text-[var(--warning)]">
              This is an estimate only and is not tax advice. Consult a qualified accountant before filing.
            </p>
          </div>
        </>
      ) : (
        <p className="text-sm text-text-secondary">Failed to load tax estimate.</p>
      )}
    </div>
  );
}
